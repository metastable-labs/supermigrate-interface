    ...
    <div className={classNames('px-8 py-6 flex flex-col self-stretch items-start rounded-2xl md:w-[440px] lg:w-[576px]', { 'gap-2 bg-primary-1450 transition-colors duration-300': current })}>
      <div className="w-full flex justify-between items-center">
        ...
        <AnimatePresence mode="popLayout">
          {current ? (
            <motion.div key="close" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="cursor-pointer" onClick={onClick}>
              <MinusIcon color="black" height={24} width={24} />
            </motion.div>
          ) : (
            <motion.div key="open" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="cursor-pointer" onClick={onClick}>
      ...
      <motion.p animate={{ height: current ? 'fit-content' : 0 }} className="text-sm md:text-base text-primary-1500">
        <AnimatePresence>
          {current && (

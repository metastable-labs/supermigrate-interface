...
const Status = ({ status }: { status: string }) => {
  ...
  const { closed, merged, pending } = locale.tokenDetail.pullRequest.status;
  return (
...
const PullRequests = ({ pullRequests, network }: { pullRequests: PullRequest[]; network: string }) => {
  const { locale } = useSystemFunctions();
  const { subtitle, title } = locale.tokenDetail.pullRequest;

  const links = pullRequests?.filter((pullRequest) => pullRequest.chain === network);

  return (
    <div className="flex flex-col gap-5">
      {links?.length > 0 && <h1 className="text-primary-50 text-[16px] leading-[24.8px] md:text-[20px] md:leading-[31px]">{title}</h1>}
        ...
        {links.map((pullRequest, index) => (
          <div key={index} className="p-4 border border-primary-2100 rounded-base flex items-center justify-between w-full bg-primary-2700">
            <div className="flex flex-col items-start gap-1.5 justify-center">
              <h3 className="text-primary-300 text-sm font-medium capitalize">{pullRequest.owner === 'optimism' || pullRequest.owner === 'iamnotstatic' ? subtitle.primary : subtitle.secondary}</h3>

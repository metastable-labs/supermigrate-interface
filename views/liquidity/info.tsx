...
const Info = ({ amount, show, token, wallet, poolPercentage, step }: { amount: number; show: boolean; wallet?: string; token?: string; poolPercentage: number; step?: number }) => {
  ...
  return (
    <AnimatePresence>
      {show && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full flex flex-col gap-3">

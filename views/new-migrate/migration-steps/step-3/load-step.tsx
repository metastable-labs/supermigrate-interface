...
const LoadStep = ({
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center gap-2">
      <div
        className={classNames('w-full flex items-center justify-center gap-2 p-2 rounded-[10px]', {
        ...
        <div
          className={classNames('rounded-full w-5 h-5 flex items-center justify-center transition-all duration-300', {
            ...
            'bg-white border border-dashed border-primary-2050 text-primary-200 shadow-small-shadow': !passed && !loading,
          })}>
          <AnimatePresence mode="popLayout">
            {!passed ? (
              <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} key={0} className="font-medium text-center text-xs">
                {step}
              </motion.span>
            ) : (
        ...
        <AnimatePresence>
          ...
          {passed && <span className="text-[10px] leading-[20px] tracking-[-0.06px] text-primary-200">{formatedDate}</span>}
        </AnimatePresence>
      </div>
